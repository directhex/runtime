...
usage()
{
  ...
  echo "  --arch                          Target platform: x86, x64, arm, armel, arm64, ppc64 or wasm."
...
while [[ $# > 0 ]]; do
  ...
  case "$opt" in
     ...
     -arch)
      ...
      case "$passedArch" in
        x64|x86|arm|armel|arm64|ppc64|wasm)
          arch=$passedArch
          ;;
        *)
          echo "Unsupported target architecture '$2'."
          echo "The allowed values are x86, x64, arm, armel, arm64, ppc64, and wasm."
          exit 1
          ;;
      esac
      shift 2
